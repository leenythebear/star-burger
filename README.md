...
[Демонстрационная версия сайта](https://starburger.work.gd/)
...
Определите переменные окружения `SECRET_KEY` и `YANDEX_API_KEY` - api-ключ, который можно получить в [кабинете разработчика](https://developer.tech.yandex.ru/). Создать файл `.env` в каталоге `star_burger/` и положите туда такой код:
```sh
...
## Запуск сайта с помощью Docker
### Dev-version

1. Сбилдите контейнеры командой:
```sh
docker build . -t <CONTAINER_NAME> -f Dockerfile_backend
docker build . -t <CONTAINER_NAME> -f Dockerfile_frontend
```
...
1. Скопируйте репозиторий в папку `/opt` на сервере.
2. Создайте файл `.env` в каталоге `/opt/star_burger/` со следующими настройками:
...
Если логгирование в Rollbar не требуется, можно указать переменную
```sh
ROLLBAR_ENABLED=False
```
в таком случае указывать переменные `ROLLBAR_TOKEN` и `CURRENT_ENVIRONMENT` не требуется

3. Запустите проект скриптом `deploy_star_burger.sh`
